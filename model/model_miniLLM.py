...
class SpongeBobModel(nn.Module):
    def __init__(self, config: SpongeBobConfig):
        ...
        self.vocab_size, self.num_hidden_layers = config.vocab_size, config.num_hidden_layers  # 词汇表大小和隐藏层数
        
        # Token Embedding: 输入 token ID 到 hidden_size 维度的嵌入
        self.embed_tokens = nn.Embedding(config.vocab_size, config.hidden_size)
        self.dropout = nn.Dropout(config.dropout)  # 输入 dropout 概率
        
        # Transformer Blocks: 堆叠多个 Transformer 块
